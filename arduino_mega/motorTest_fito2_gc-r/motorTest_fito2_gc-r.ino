...
int angleLimit = 170;
int sweepLimit = 35;
int angleLimitShyState = 145;
int angleCorrection = 90;
int shortArmStopShyAngle = 105;
bool armState[totalElements];
int stateIndex = 0;
int isItShyCounter = 0;
int shyCycleLimit = 120; //measured in the system's cycles --> about 1 second each cycle

bool ison = true;
int generalCycle = 0;
int generalLoopLimit = 5; // default use 200 //// measured in the system's cycles --> about 1 second each cycle
int halt = 10000;// for 8 mins use 480000
//----------------------------------
void setup() {
  Serial.begin(9600);
...
void loop() {
  if (ison) {
    ...
    if (isItShyCounter < shyCycleLimit) {
      ...
      runShyState(6, halt);//8 mins delay TOTAL HALT OF THE SYSTEM
    ...
    for (int i = 0; i <= totalServos - 1; i += 2) {
      if (armState[stateIndex] == false) {
        if (servoPins[i] == 2 || servoPins[i] == 4 || servoPins[i] == 6) {
          ...
          servos[i].write(servoPos + angleCorrection + sweepLimit);
          servos[i + 1].write(angleLimit - servoPos - angleCorrection - sweepLimit);
          delay(inc);
        }
      } else {
  ...
  for (servoPos = 10; servoPos <= angleLimitShyState; servoPos += 1) {
    for (int i = 0; i <= totalServos - 1; i += 2) {
      if (servoPins[i] == 2 || servoPins[i] == 4 || servoPins[i] == 6) {
        ...
        if (servoPos > sweepLimit && servoPos < shortArmStopShyAngle) {
          servos[i + 1].write(servoPos);
          servos[i].write(angleLimit - servoPos);
        }
        delay(inc);
  ...
  for (servoPos = angleLimitShyState; servoPos >= servoInitPos; servoPos -= 1) {
    for (int i = 0; i <= totalServos - 1; i += 2) {
      if (servoPins[i] == 2 || servoPins[i] == 4 || servoPins[i] == 6) {
        ...
        if (servoPos < shortArmStopShyAngle && servoPos > sweepLimit) {
          servos[i + 1].write(servoPos);
          servos[i].write(angleLimit - servoPos);
        }
        delay(slowInc);
